...
int main(void){
    ...
    // for 문 이용
    int i; // 두번째 for문에 있는 i에 가장 마지막으로 저장된 값을 세번째 for문에서 사용하기 위함
    int total = N; // N 값 저장시켜 놓기 (이유: 첫번째 for문에 의해서 N값은 계속 변경됨)
...
/*
    ...
    2. for(; N>0; N--)
       => ++ 되게 고치기
    3. for(int j=0; j<total-i; j++)
       => N개만큼 별 출력되게 하기
       => for(int j=0; j<N; j++) ?
    4. 왠만하면 변수 통일하기
       => 예: for문 계속 돌리면 for문에서 계속 i 선언 후 동일한 i 계속 사용
*/
...
/*
    ...
    6. 중간 점검 (2)
       실행 결과: 왼쪽 위부터 출력되며 공백 개수 적어지나 별 개수는 1개로 유니
                 ____*
                 ___*
                 __*
                 _*
